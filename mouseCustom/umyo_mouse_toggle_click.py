...
mouse_held = False  # Track if mouse is currently held down
click_toggled = False  # Track if we've processed the toggle for this activation
...
def print_mode_info():
    ...
    mode_text = "Up Arrow Mode" if USE_ARROW_MODE else "Toggle Click Mode"
    print(f"Current mode: {mode_text}")
    if USE_ARROW_MODE:
        print("EMG activation will press UP ARROW key")
    else:
        print("EMG activation will TOGGLE MOUSE CLICK (press/release)")
    print("="*40)
...
while (1):
    ...
    if (cnt > 0):
        ...
        if (ch1 > fix_TH1):
            ...
            mouse_click_active = 0
            click_toggled = False  # Reset toggle flag when not active
        ...
        if (calibrate_requested == 0):
            ...
            if (mouse_click_active > 0):
                # Toggle click mode: press or release mouse button
                if not click_toggled:
                    if mouse_held:
